#!/usr/bin/env python3
"""us-trip 離線唯讀／Service Worker 端對端測試。

不連真實資料庫：localStorage 預先塞一個不存在的 Firebase 網址，apiKey 留空。
每次執行都把 us-trip.html 複製成暫存目錄的 index.html（us-trip-sw.js 存在時複製成 sw.js），
用本機 http server 提供，因為 Service Worker 只在 http(s)/localhost 上運作，playwright 也擋 file:。

用法：
  python3 scripts/test_us_offline.py            # 全部
  python3 scripts/test_us_offline.py selftest   # 只跑指定測試
"""
import http.server
import json
import shutil
...
def new_page(browser, api_key='', seed=True):
    ...
    if seed:
        ...
        ctx.add_init_script(
            "if(!localStorage.getItem('us_trip')){"
            f"localStorage.setItem('us_trip',{json.dumps(dev)});"
            f"localStorage.setItem('us_trip_snap',{json.dumps(json.dumps(SNAP))});}}"
        )
    page = ctx.new_page()
    page.dialogs = []
...
def test_sw_offline(browser):
    ...
    page.wait_for_selector('#app', state='visible')
    wait_sw(page)
    n_lib = page.evaluate("caches.open('lib-u0915a').then(c=>c.keys()).then(k=>k.length)")
    ...
    # us-trip 開啟時預設跳到「今天」那天；SNAP 的活動在 day1，先切過去，避免結果隨執行日期變動
    page.click('#ds-day1')
